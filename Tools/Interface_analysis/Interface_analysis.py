...
def analyze_uris_in_batches(uri_list, batch_size=10):
    all_results = []
...
def print_results(all_uris):
    ...
    for result in sorted_uris:
        ...
        output += f"{risk_score}（{visual_label}） {uri} [{response_size}] 可能存在{vulnerability}，原因为{explanation}\n"
        print(f"{color}{risk_score}（{visual_label}） {uri} [{response_size}] 可能存在{vulnerability}，原因为{explanation}{RESET}")
