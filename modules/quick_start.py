...
class QuickStart(ttk.Frame):
    ...
    def __init__(self, parent, main_app):
        ...
        self.current_directory = main_app.config.get('current_directory', '')
    ...
    def init_variables(self):
        ...
        self.regular_infomation_path = self.main_app.config.get('Regular_Expression_Extraction', '')
    ...
    def create_widgets(self):
        ...
        # 正则提取数据
        self.regular_infomation_frame = ttk.LabelFrame(self.main_frame, text="正则提取模块", padding="3", style="Border.TFrame")
        self.regular_infomation_frame.grid(row=2, column=0, sticky="nsew", padx=5, pady=5)
        # 导入内容按钮
        self.regular_infomation_input_button = ttk.Button(
            self.regular_infomation_frame, text="导入内容", command=self.regular_infomation_import_content, style="ButtonTheme.TButton"
        )
        self.regular_infomation_input_button.pack(side='left',padx=5, pady=5,anchor='nw')
        # 开始提取
        self.regular_infomation_start_button = ttk.Button(
            self.regular_infomation_frame, text="开始提取", command=self.regular_infomation_start, style="ButtonTheme.TButton"
        )
        self.regular_infomation_start_button.pack(side='left',padx=10, pady=5, anchor='nw')
        # 打开提取结果
        self.regular_infomation_result_button = ttk.Button(
            self.regular_infomation_frame, text="打开提取结果", command=self.regular_infomation_result, style="ButtonTheme.TButton"
        )
        self.regular_infomation_result_button.pack(side='left', padx=10, pady=5, anchor='nw')
        # 正则提取使用手册
        self.regular_infomation_readme_button = ttk.Button(
            self.regular_infomation_frame,
            text="使用手册",
            command=self.open_readme,
            style="ButtonTheme.TButton"
        )
        self.regular_infomation_readme_button.pack(side='left', padx=10, pady=5, anchor='nw')
    ...
    # 正则提取需要先写入内容
    def regular_infomation_import_content(self):
        file_path = os.path.join(self.current_directory, 'Tools/Regular_Expression_Extraction/data.txt')
        cmd = f'"{self.notepad_path}" "{file_path}"'
        if os.path.exists(file_path):
            try:
                subprocess.Popen(cmd)
                ...
                messagebox.showerror("启动失败", f"启动进程时出错：{e}")
    # 开始提取
    def regular_infomation_start(self):
        file_path = os.path.abspath(os.path.join(self.regular_infomation_path, 'Regular_Expression_Extraction.py'))
        python_path = os.path.abspath(self.python_path)
        if os.path.exists(file_path):
            print("开始扫描")
            cmd = f'start "" cmd /k "{python_path} {file_path}"'
            print(f"执行命令: {cmd}")
            try:
                subprocess.Popen(cmd, cwd=self.regular_infomation_path, shell=True)
            except Exception as e:
                messagebox.showerror("启动失败", f"启动进程时出错：{e}")
        else:
            messagebox.showerror("文件不存在", f"文件 {file_path} 不存在，请检查路径是否正确。")
    # 打开提取结果
    def regular_infomation_result(self):
        file_path = os.path.join(self.regular_infomation_path, 'result.txt')
        if self.notepad_path=="notepad++":
            cmd = [self.notepad_path, "-ro", file_path]
        else:
            cmd = [self.notepad_path, file_path]
